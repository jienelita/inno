...
const Create = () => {
    ...
    //     setFrontendErrors({ ...frontendErrors, [e.target.name]: null });
    // };
    ...
    const checkCID = async (value: string) => {
        ...
        try {
            ...
            setCidAvailable(data.available); // assume backend returns { available: true/false }
        } catch (err) {
            setCidAvailable(null); // handle error case
        } finally {
    ...
    const validateStep = () => {
        const newErrors = {};
        if (step === 1) {
            if (!form.cid) newErrors.cid = 'CID is required.';
    ...
    const showError = (field) => frontendErrors[field] || backendErrors[field];
    return (
